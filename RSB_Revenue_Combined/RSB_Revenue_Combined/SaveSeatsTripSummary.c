...
//		"Referer={saveSeatReferrer}", 
	...
	if(atoi(lr_eval_string("{redirect_count}"))>0){
	...
	} else if(atoi(lr_eval_string("{success_count}"))<=0)
		{
			...
			lr_output_message("RSB Seat Selection in Trip Summary Page Failed: \tCartID:%s\tCacheKey:%s\tResponse:%s",lr_eval_string("{cartId}"), lr_eval_string("{cacheKey}"), lr_eval_string("{wholeRequest}"));			
...
//		LAST);
	...
	{
		lr_output_message("RSB Save Seats in Trip Summary Page unsuccessful: \tCartID:%s\tCacheKey:%s",lr_eval_string("{cartId}"), lr_eval_string("{cacheKey}"));
		lr_end_transaction(lr_eval_string("RSB_Revenue_{RandTripType}_09_SaveSeats"), LR_AUTO);
		lr_end_transaction("%RSB_TripSummary%", LR_AUTO);	
		lr_exit(LR_EXIT_ITERATION_AND_CONTINUE, LR_AUTO);
	}
